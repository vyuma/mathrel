...
//! # どちらを使うべきか
//!
//! **数式セルと証明を一緒に扱うなら [`mathrel_host::Workspace`] を使うこと。**
//! そちらは同じグラフに数式セルも載るので、`x = 2` というセルと `x` に言及
//! する定理が繋がる（issue #22 で統合した）。
//!
//! この `ProofSpace` は、ホスト層（パーサと評価器）を持ち込まずに検証層だけを
//! 使いたい場合の入口である。定義は Lean のソースを文字列で直に渡す。
//! 2 つを併用すると**グラフが 2 つになる**ので、混ぜないこと。
//!
